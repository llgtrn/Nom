...
> **HEAD:** wave-11 landing (**1429 workspace tests** green across **13 crates** under `RUSTFLAGS="-D warnings -A deprecated" NOM_SKIP_GPU_TESTS=1`). Wave-11 completes the Rgba-disambiguation triad (nom-gpui Rgba → LinearRgba + Hsla::to_linear_rgba across 9 files in 3 crates) and adds 9 modules: nom-compose/cost_estimator (13-kind defaults), nom-editor/find_replace (pure-str scan), nom-editor/indent (detect_style + indent/dedent/depth), nom-panels/diagnostics_panel, nom-panels/settings_panel, nom-gpui/scene_builder (auto DrawOrder), nom-canvas-core/groups (GroupRegistry with single-group membership), nom-lint/rules/+2 (no_tab_indent_after_space + require_trailing_newline), cross_workspace_smoke test. | **Date:** 2026-04-17
