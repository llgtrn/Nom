...
> **HEAD:** wave-8 LANDING (**1028 workspace tests** green under `RUSTFLAGS="-D warnings"` across **13 crates**). Wave-8 added 10 Phase 4 backend stubs to `nom-compose/backends/`: video, image, web_screen, native_screen, data_extract, data_query, storyboard_narrative (5-phase + 4-phase pipelines), audio, data_frame (Polars MVP pattern, no external deps), mesh (glTF 2.0). All 11 NomKind variants now have stub backends via `register_all_stubs()`. | **Date:** 2026-04-17
