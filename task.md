...
> **HEAD:** wave-9 LANDING (**1130 workspace tests** green under `RUSTFLAGS="-D warnings"` across **13 crates**). Wave-9: 2 audit-finding fixes (drawing::Rgba → SrgbColor, highlight::Rgba → PackedRgba, FractionalIndex moved to block_model, ImageSpec::final_dimensions saturating_mul + MAX_FINAL_DIMENSION guard, xy_cut symmetric recursion) + scenario_workflow backend + plugin_registry + 2 integration tests (dispatcher_end_to_end + graph-v2 end_to_end) + nom-gpui cursor + nom-panels shortcuts + nom-blocks tree_query + nom-theme integration tests. All 11 NomKind backends registered; 0 brand-name violations confirmed by audit. | **Date:** 2026-04-17
