...
### 5.1 The `intended to` clause is the purpose declaration

Every `.nomx` sentence generated by the AI **must** contain an `intended to <purpose>` clause immediately below the title line. This is not optional prose — it is the machine-readable purpose declaration that drives promotion.

```nomx
the media nano_banana_promo is
  intended to compose a 30-second promotional video for brand awareness.
  uses the @media matching 'brand footage' with at-least 0.75 confidence.
  composes intro_scene, product_shot, call_to_action.
  outputs video/mp4.
```

The `intended to` line maps to a `clause_shapes` row with `verb = "intended_to"`. Its content becomes the `description` field of the `grammar.kinds` row on promotion. Any AI-generated sentence missing this clause is rejected before execution — the orchestrator retries with an explicit prompt requiring it.

### 5.2 Two promotion paths
...
Stage 0 — Transient
  ...
  UI badge: ⚡ (amber)
      ...
      ┌──────────────────────────────┬─────────────────────────────────┐
      │  EXPLICIT PATH               │  AUTO PATH                      │
      │  User confirms purpose       │  User never reviews             │
      │                              │                                 │
      │  User clicks Accept OR       │  usage_count >= 3               │
      │  edits .nomx and saves       │  AND confidence >= 0.7          │
      │  → purpose is declared       │  (background ticker, 60s poll)  │
      └──────────────┬───────────────┴──────────────┬──────────────────┘
                     │                              │
                     ▼                              ▼
Stage 1 — Partial  (promoted immediately on explicit; or auto-threshold on background)
  DictWriter::insert_partial_entry(word, kind, nomx_code, purpose, confidence)
  grammar.kinds row: status=Partial, source=ai_generated
  description = extracted "intended to <purpose>" text
...
Stage 2 — Complete  [trigger: used 10+ times AND passes compiler validation]
  ...
  grammar.kinds row: status=Complete
  UI badge: ● (filled, indistinguishable from human-authored)
  Palette entry moves from "AI" group into regular kind group
```

### 5.3 What "user confirms purpose" means

The AI Review card in the right dock shows the generated `.nomx` sentence with the `intended to` clause highlighted. The user has three choices:

- **Accept** — purpose is confirmed as-is → `DictWriter::insert_partial_entry()` fires immediately
- **Edit grammar** — user rewrites the `intended to` clause inline, saves → purpose is confirmed with user's wording → `insert_partial_entry()` fires immediately with edited text
- **Skip** — no purpose confirmation → stays Transient; auto-path takes over if used repeatedly

This means: **the purpose statement, written clearly below the entity title, is the promotion trigger — not a usage count.** Usage count is only the safety net for cases where the user never interacts with the review card.

### 5.4 Promotion thresholds

Stored in `glue_promotion_config` DB table — not Rust constants. Initial seed:

| key | value | meaning |
|---|---|---|
| auto_promote_count | 3 | usage threshold for background auto-promotion |
| auto_promote_confidence | 0.7 | minimum confidence for auto-promotion |
| complete_use_count | 10 | uses needed to promote Partial → Complete |
| complete_requires_validation | true | compiler validation required before Complete |
...
Background promotion ticker polls every 60 seconds for auto-path candidates.

**Key invariant:** every entity gets a `NomtuRef` on first promotion to Partial — whether via explicit or auto path. The `entity: NomtuRef` non-optional mandate is maintained.
...
The `intended to` clause is surfaced prominently as the **purpose statement** — the one line the user must confirm or rewrite. Everything else is secondary.
...
│  the media nano_banana_promo is      │
│ ┌────────────────────────────────┐   │
│ │ intended to compose a          │   │  ← purpose clause, editable inline
│ │ 30-second promotional video    │   │     highlighted with accent border
│ │ for brand awareness.           │   │
│ └────────────────────────────────┘   │
│  uses @media · composes 3 steps      │
│  outputs video/mp4                   │
│                                      │
│  [Accept]  [Edit purpose]  [Skip]    │
└──────────────────────────────────────┘
```

**Accept** → purpose confirmed as-is → `DictWriter::insert_partial_entry()` fires immediately. No usage count needed.
**Edit purpose** → inline edit of the `intended to` clause only; save → confirmed with user's wording → `insert_partial_entry()` fires immediately.
**Skip** → stays Transient; auto-path takes over if used 3+ more times at 70%+ confidence.

The purpose text extracted from the `intended to` clause becomes the `description` field of the `grammar.kinds` row — shown in the node palette tooltip, graph node subtitle, and LSP hover.
