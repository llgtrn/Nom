...
/// An ordered chain of pipeline components forming a complete pipeline.
#[derive(Debug, Default)]
pub struct ComponentPipeline {
    pub stages: Vec<PipelineComponent>,
}
...
impl ComponentPipeline {
    ...
    pub fn add_stage(&mut self, stage: PipelineComponent) {
    ...
    pub fn retrieval_stages(&self) -> Vec<&PipelineComponent> {
        self.stages
            .iter()
...
mod component_pipeline_tests {
    ...
    #[test]
    fn pipeline_component_set_and_get_config() {
        let mut c = PipelineComponent::new("dense", ComponentType::Retriever);
    ...
    #[test]
    fn pipeline_component_is_retrieval_stage_true() {
        let r = PipelineComponent::new("r", ComponentType::Retriever);
        let rd = PipelineComponent::new("rd", ComponentType::Reader);
        assert!(r.is_retrieval_stage());
        assert!(rd.is_retrieval_stage());
    }

    #[test]
    fn pipeline_component_is_retrieval_stage_false() {
        let ranker = PipelineComponent::new("ranker", ComponentType::Ranker);
        let gen = PipelineComponent::new("gen", ComponentType::Generator);
        let flt = PipelineComponent::new("flt", ComponentType::Filter);
    ...
    fn component_pipeline_add_and_count() {
        ...
        p.add_stage(PipelineComponent::new("s1", ComponentType::Retriever));
        p.add_stage(PipelineComponent::new("s2", ComponentType::Ranker));
    ...
    fn component_pipeline_pipeline_signature() {
        let mut p = ComponentPipeline::new();
        p.add_stage(PipelineComponent::new("retrieve", ComponentType::Retriever));
        p.add_stage(PipelineComponent::new("rank", ComponentType::Ranker));
        p.add_stage(PipelineComponent::new("read", ComponentType::Reader));
    ...
    fn component_pipeline_retrieval_stages() {
        let mut p = ComponentPipeline::new();
        p.add_stage(PipelineComponent::new("r1", ComponentType::Retriever));
        p.add_stage(PipelineComponent::new("ranker", ComponentType::Ranker));
        p.add_stage(PipelineComponent::new("r2", ComponentType::Reader));
