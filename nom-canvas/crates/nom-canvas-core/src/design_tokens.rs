...
//!
//! Encodes design tokens as typed Rust values with category
...
/// A single design token with a name, value, and category.
#[derive(Debug, Clone)]
...
impl DesignToken {
    ...
    ///
    /// Example: token `"primary"` → `"--design-primary"`.
    pub fn css_var(&self) -> String {
        format!("--design-{}", self.name)
    }
}

/// The full set of design tokens.
...
impl DesignTokenSet {
    ...
    /// Seeds the token set with 9 representative default tokens.
    ///
    ...
    pub fn seed_defaults(&mut self) {
        ...
        for (name, value, cat) in defaults {
            self.tokens.push(DesignToken::new(name, value, cat));
        }
    }

    /// Finds a token by exact name, returning `None` if not present.
    pub fn find(&self, name: &str) -> Option<&DesignToken> {
        self.tokens.iter().find(|t| t.name == name)
    }

    /// Returns all tokens belonging to the given category.
    pub fn by_category(&self, cat: &TokenCategory) -> Vec<&DesignToken> {
...
impl TokenResolver {
    ...
    pub fn new(token_set: DesignTokenSet) -> Self {
        Self { token_set }
    }
...
#[cfg(test)]
mod design_tokens_tests {
    use super::*;

    fn seeded_set() -> DesignTokenSet {
        let mut s = DesignTokenSet::new();
        s.seed_defaults();
        s
    }
    ...
    #[test]
    fn design_token_is_color() {
        let t = DesignToken::new("primary", "#1971FA", TokenCategory::Color);
        assert!(t.is_color(), "color token must report is_color() = true");

        let s = DesignToken::new("spacing-md", "16px", TokenCategory::Spacing);
        assert!(!s.is_color(), "spacing token must report is_color() = false");
    }

    #[test]
    fn design_token_css_var() {
        let t = DesignToken::new("border-radius", "8px", TokenCategory::Border);
        assert_eq!(t.css_var(), "--design-border-radius");
    }

    #[test]
    fn design_token_set_seed_defaults_count() {
        let s = seeded_set();
        assert_eq!(s.token_count(), 9, "seed_defaults must insert exactly 9 tokens");
    }

    #[test]
    fn design_token_set_find_existing() {
        let s = seeded_set();
    ...
    #[test]
    fn design_token_set_by_category_color() {
        let s = seeded_set();
    ...
    fn token_resolver_resolve_css_var() {
        let r = seeded_resolver();
        assert_eq!(r.resolve_css_var("font-body"), "var(--design-font-body)");
