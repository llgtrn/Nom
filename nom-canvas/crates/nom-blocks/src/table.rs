...
impl TableBlock {
    pub fn new(entity: NomtuRef, columns: Vec<TableColumn>) -> Self {
        Self { entity, columns, rows: Vec::new() }
    }
    pub fn add_row(&mut self, row: Vec<SlotValue>) -> Result<(), String> {
        if row.len() != self.columns.len() {
            return Err(format!("Row has {} cells, expected {}", row.len(), self.columns.len()));
        }
        self.rows.push(row);
        Ok(())
    }
...
mod tests {
    ...
    fn table_row_validation() {
        ...
        let cols = vec![
            TableColumn { id: "c1".into(), name: "Name".into(), col_type: "text".into(), width: None },
            TableColumn { id: "c2".into(), name: "Age".into(), col_type: "number".into(), width: None },
        ];
        let mut t = TableBlock::new(entity, cols);
        assert!(t.add_row(vec![SlotValue::Text("Alice".into()), SlotValue::Number(30.0)]).is_ok());
