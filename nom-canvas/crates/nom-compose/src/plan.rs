...
mod tests {
    ...
    fn plan_add_step_returns_sequential_ids() {
        ...
        let a = plan.add_step("video", "in", "v_out");
        let b = plan.add_step("audio", "in", "a_out");
        assert_eq!(a, 0);
        assert_eq!(b, 1);
    }
    #[test]
    fn plan_topo_order_respects_deps() {
        ...
        let b = plan.add_step_after("export", "v", "out", vec![a]);
    ...
    fn plan_is_valid_dag_for_linear_chain() {
        let mut plan = CompositionPlan::new();
        let a = plan.add_step("video", "s", "v");
        plan.add_step_after("export", "v", "o", vec![a]);
    ...
    fn plan_step_count() {
        let mut plan = CompositionPlan::new();
        plan.add_step("video", "in", "v");
        plan.add_step("audio", "in", "a");
        plan.add_step("export", "a", "out");
    ...
    fn plan_step_label_preserved() {
        let mut plan = CompositionPlan::new();
        plan.add_step("image", "source_image", "rendered_image");
    ...
    fn plan_step_execution_order() {
        let mut plan = CompositionPlan::new();
        let a = plan.add_step("video", "src", "v");
        let b = plan.add_step("audio", "src", "a");
        let c = plan.add_step("export", "a", "out");
        let order = plan.topo_order();
    ...
    fn plan_add_step_increases_step_count() {
        ...
        plan.add_step("image", "in", "out");
        assert_eq!(plan.steps.len(), 1);
        plan.add_step("audio", "in", "a_out");
    ...
    fn plan_step_depends_on_preserved() {
        let mut plan = CompositionPlan::new();
        let a = plan.add_step("audio", "in", "a");
        let b = plan.add_step_after("export", "a", "out", vec![a]);
    ...
    fn plan_step_ids_match_indices() {
        let mut plan = CompositionPlan::new();
        for i in 0..6usize {
            let id = plan.add_step("render", "i", "o");
    ...
    fn plan_large_linear_chain_topo_order() {
        let mut plan = CompositionPlan::new();
        let first = plan.add_step("video", "in", "s0");
        let mut prev = first;
        for i in 1..20usize {
    ...
    fn plan_one_step_topo_order_contains_that_step() {
        let mut plan = CompositionPlan::new();
        let id = plan.add_step("audio", "src", "out");
        let order = plan.topo_order();
    ...
    fn plan_ten_steps_linear_all_appear_in_order() {
        let mut plan = CompositionPlan::new();
        let first = plan.add_step("video", "raw", "s0");
        let mut prev = first;
        for i in 1..10usize {
            prev = plan.add_step_after(
                "transform",
        ...
        for (expected, &actual) in order.iter().enumerate() {
            assert_eq!(
                actual, expected,
                "step at position {expected} must be {expected}"
            );
        }
    }
    ...
    fn plan_step_backend_field_preserved() {
        let mut plan = CompositionPlan::new();
        plan.add_step("render", "x", "y");
        assert_eq!(plan.steps[0].backend, "render");
    }

    #[test]
    fn plan_step_input_output_keys_preserved() {
        let mut plan = CompositionPlan::new();
        plan.add_step("export", "my_input", "my_output");
    ...
    fn plan_depends_on_empty_for_root_steps() {
        let mut plan = CompositionPlan::new();
        plan.add_step("video", "a", "b");
        plan.add_step("audio", "c", "d");
    ...
    fn plan_two_independent_steps_both_in_topo() {
        let mut plan = CompositionPlan::new();
        let a = plan.add_step("video", "v_in", "v_out");
        let b = plan.add_step("audio", "a_in", "a_out");
        let order = plan.topo_order();
    ...
    fn plan_step_id_equals_insertion_index() {
        let mut plan = CompositionPlan::new();
        for i in 0..5usize {
            let id = plan.add_step("transform", "x", "y");
            assert_eq!(id, i);
    ...
    // ── Wave AE new tests (second block) ────────────────────────────────────
    ...
    fn plan_zero_steps_topo_is_empty_and_dag_is_valid() {
        ...
        assert!(
            order.is_empty(),
            "zero-step plan must have empty topo order"
        );
    ...
    fn plan_step_insertion_order_preserved_in_steps_vec() {
        ...
        for k in &keys {
            plan.add_step("transform", *k, "out");
        }
        for (i, k) in keys.iter().enumerate() {
            assert_eq!(
                plan.steps[i].input_key, *k,
                "step {i} input key must be '{k}'"
            );
        }
    }
    ...
    fn plan_round_trip_fields_via_clone() {
        let mut plan = CompositionPlan::new();
        let a = plan.add_step("video", "v_src", "v_out");
        let b = plan.add_step_after("export", "v_out", "exported", vec![a]);
        let rt = plan.clone();
    ...
    fn plan_large_step_count_topo_order_contains_all() {
        let mut plan = CompositionPlan::new();
        for _ in 0..50 {
            plan.add_step("transform", "in", "out");
        }
    ...
    fn plan_step_ordering_respected_in_three_level_chain() {
        let mut plan = CompositionPlan::new();
        let a = plan.add_step("video", "raw", "v");
        let b = plan.add_step_after("audio", "v", "av", vec![a]);
        let c = plan.add_step_after("export", "av", "final", vec![b]);
        let order = plan.topo_order();
    ...
    fn plan_parallel_then_merge_ordering() {
        let mut plan = CompositionPlan::new();
        let a = plan.add_step("video", "v", "v_out");
        let b = plan.add_step("audio", "a", "a_out");
        let c = plan.add_step_after("export", "merged", "final", vec![a, b]);
        assert!(plan.is_valid_dag());
        let order = plan.topo_order();
        let pos = |id| order.iter().position(|&x| x == id).unwrap();
    ...
    fn plan_add_step_after_with_multiple_deps_preserves_all() {
        let mut plan = CompositionPlan::new();
        let a = plan.add_step("video", "a", "va");
        let b = plan.add_step("audio", "b", "ab");
        let c = plan.add_step("image", "c", "ic");
        let d = plan.add_step_after("export", "combo", "out", vec![a, b, c]);
    ...
    fn plan_single_step_is_dag_and_topo_contains_it() {
        let mut plan = CompositionPlan::new();
        let id = plan.add_step("render", "in", "out");
        assert!(plan.is_valid_dag());
        let order = plan.topo_order();
        assert_eq!(order, vec![id]);
    }
    ...
    fn plan_step_count_after_3_adds() {
        ...
        plan.add_step("video", "a", "b");
        plan.add_step("audio", "c", "d");
        plan.add_step("export", "e", "f");
        assert_eq!(
            plan.steps.len(),
            3,
            "three add_step calls must yield 3 steps"
        );
    }
    ...
    fn plan_two_step_chain_topo_preserves_order() {
        let mut plan = CompositionPlan::new();
        let a = plan.add_step("image", "src", "img");
        let b = plan.add_step_after("document", "img", "doc", vec![a]);
        let order = plan.topo_order();
    ...
    fn plan_step_depends_on_multiple_parents_all_recorded() {
        ...
        let a = plan.add_step("video", "v", "v_out");
        let b = plan.add_step("audio", "a", "a_out");
        let c = plan.add_step("image", "i", "i_out");
        let d = plan.add_step_after("export", "combo", "final", vec![a, b, c]);
        assert_eq!(plan.steps[d].depends_on, vec![a, b, c]);
    ...
    fn plan_topo_contains_all_step_ids() {
        ...
        let ids: Vec<usize> = (0..7)
            .map(|_| plan.add_step("transform", "x", "y"))
            .collect();
        let order = plan.topo_order();
        for id in &ids {
    ...
    fn plan_is_valid_dag_true_for_tree() {
        let mut plan = CompositionPlan::new();
        let a = plan.add_step("video", "v", "v_out");
        let b = plan.add_step_after("audio", "v_out", "a_out", vec![a]);
        let c = plan.add_step_after("image", "v_out", "i_out", vec![a]);
        let _ = plan.add_step_after("export", "merged", "final", vec![b, c]);
    ...
    fn plan_output_key_preserved_per_step() {
        let mut plan = CompositionPlan::new();
        plan.add_step("render", "input_frame", "rendered_frame");
        plan.add_step("export", "rendered_frame", "export_artifact");
