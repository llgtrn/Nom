...
fn golden_type_nomx_gets_highlight_tokens() {
    ...
    // kinds, so is_known_kind returns false — confirms the pipeline is wired.
    let source_word = "define";
    let result = ui.is_known_kind(source_word);
    // Result is a bool — the pipeline returned without panic
    let _ = result;
    // grammar_keywords returns an empty vec on a fresh bridge (no DB populated)
    let tokens = ui.grammar_keywords();
    // Not asserting non-empty because no DB is loaded; assert type is correct
    let _: Vec<String> = tokens;
}
