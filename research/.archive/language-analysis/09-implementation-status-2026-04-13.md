...
> **Archive snapshot — finalized 2026-04-14.** 2026-04-13 status snapshot; doc 08 carries current state.
...
> **2026-04-14 grammar-registry complete + dict-split S1-S2 landed:** HEAD `bfd2d05`. **Grammar registry fully populated** — new crate `nom-grammar` (commit `b53f74c` P1 schema → `32a9502` P4+P5 kinds+qualities+authoring_rules → `ee37edd` P2 keywords → `bfd2d05` P3 clause_shapes) now ships `grammar.sqlite` with **9 kinds + 10 quality_names + 45 keywords + 46 clause_shapes + 450+ authoring_rules** — every syntactic layer queryable by AI clients without reading .md prose. CLI: `nom grammar init` + `nom grammar seed` + `nom grammar status`. 17/17 nom-grammar tests pass (schema init, keyword seed, clause-shape coverage across all 9 kinds, required-clause spot checks, doc-16 markdown parse round-trip, idempotency checks on all 4 seed functions). **Dict-split S1+S2 shipped** (commits `ff09fac` + `dd91ff5`) — `Dict { concepts: Connection, words: Connection }` struct with `open_dir(&Path)` / `open_paths(&Path, &Path)` / `open_in_memory()` + per-tier CONCEPTS_SCHEMA_SQL (DB1 tables only) + WORDS_SCHEMA_SQL (DB2 tables only); cross-file FK absent per doc 22 §1. 48/48 nom-dict tests pass. Three physical SQLite files now legible: `concepts.sqlite` + `words.sqlite` + `grammar.sqlite` (plus `~/.nom/store/<hash>/` artifact tree). Remaining S3-S8 (dict-split function port through legacy removal) queued for upcoming /loop cycles.
>
> **2026-04-14 K-framework refresh:** HEAD `4820144` (post-doc-10-K-ledger). Corpus counts bumped again: doc 14 now carries **87 translations / 74 paradigm families** (survey target exceeded by 48%; up from 86/73 at `067f18c`). Doc 16 backlog at **393 closed / 463 rows = 84.9%** (up from 385/455). Paradigm tail: **46 consecutive 0-new-wedge translations** (Dafny #50 through K-framework #87 — 38-member streak). **Formal-methods paradigm family now 7 exemplars**: Hypothesis #33 + TLA+ #47 + PDDL #48 + Dafny #50 + Idris #73 + Coq #86 + **K framework #87 (term-rewriting operational semantics)**. **Deepest unifying insight documented**: formal-semantics frameworks + theorem provers + model checkers + testing frameworks all reduce to the same Nom surface — a named property with generator over data decls + cited peer functions/properties via typed slots + `ensures` clauses; **only the `ensures` vocabulary varies** (reduces-to / satisfies-LTL / proof-discharged-by / holds-for-all-generated-inputs); the structural skeleton is identical. This is the strongest evidence to-date that the `.nomx v2` primitive set is paradigm-universal.
>
> **2026-04-14 very-very-late refresh:** HEAD `067f18c` (post-Coq-tactics-closures). Corpus counts bumped again: doc 14 now carries **86 translations / 73 paradigm families** (survey target exceeded by 46%; up from 84/71 at `459aa1e`). Doc 16 backlog at **385 closed / 455 rows = 84.6%** (up from 369/439). Paradigm tail: **45 consecutive 0-new-wedge translations** (Dafny #50 through Coq-tactics #86 — 37-member streak). Two new paradigm-family additions: **fault-tolerance (3 exemplars: GenServer #27 + retry-policy W43 + Erlang OTP supervisor #85)** and **formal-methods now 6 exemplars (Hypothesis #33 + TLA+ #47 + PDDL #48 + Dafny #50 + Idris #73 + Coq #86)**. Unifying insight from Coq translation: Nom treats mechanized proofs as declarative contracts, not procedural scripts — author names induction principle + peer lemmas via @Property typed slots; build-stage proof engine handles tactic application; ~80% of procedural-ritual authoring burden eliminated. 10/10 QualityName seeds fixed-registered. **4 invariant principles documented**: density-inversion + non-local-control-flow + explicit-type-per-value + functions-describe-what-they-request-handlers-describe-what-requests-do.
>
> **2026-04-14 very-late refresh:** HEAD `459aa1e` (post-doc-05-banner-refresh). Corpus counts bumped: doc 14 now carries **84 translations / 71 paradigm families** (survey target exceeded by 42%; up from 80/67 at `cf5eb51`). Doc 16 backlog at **369 closed / 439 rows = 84%** (up from 342/412 = 83%). Paradigm tail: **43 consecutive 0-new-wedge translations** (Dafny #50 through OCaml 5 algebraic effects #84). Effect-handling family now has 4 exemplars (Haskell monads + F# computation exprs + Scheme call/cc + OCaml effects). 10/10 QualityName seeds fixed-registered. **4 invariant principles documented**: density-inversion + non-local-control-flow + explicit-type-per-value + functions-describe-what-they-request-handlers-describe-what-requests-do. Doc 03 carries paradigm-coverage enablement banner (commit `a3c95f3`) citing specific translations that enable self-hosting at the authoring layer. Doc 05 verified-banner refreshed to HEAD `a3c95f3` (commit `459aa1e`).
>
> **Prior 2026-04-14 end-of-session refresh:** HEAD `cf5eb51`. **Session arc** since `fdfb32d`: 275+ commits across four macro-lanes. (1) **CoreNLP-inspired W4 strictness lane** — 5 of 6 wedges closed (A1/A2/A3/A4/A6; A5 audited with refactor deferred). A4 shipped as the full S1-S6 annotator pipeline. See doc 13 §5 + doc 18. (2) **Research finalization lane** — docs 13-20 landed: strictness plan, Accelworld translations (**80-milestone** spanning **67 paradigm families**; 50-language-survey exceeded by 34%), 100-repo harness plan, **412-row** gap backlog triage (342 closed, 83%), **complete authoring-guide chapter I1-I20**, annotator-pipeline design, deferred-design resolutions, session summary. **0 open design questions**. (3) **Pipeline parity push (a4c20-a4c38)** — every observable field from `parse_nom`/`parse_nomtu` now covered by `run_pipeline`. Delegate migration fully unblocked. **nom-concept test total: 139**. (4) **Paradigm-coverage proof** — **47 consecutive minimal-wedge translations, 39 consecutive 0-new-wedge**. Closed kind set grew 7→9 (`property` + `scenario`). **20+ paradigm families fully closed / unified**: abstraction-quadrant (macros + generics + typeclasses + functors), formal-methods-quadrant (gen-testing + model-checking + AI-planning + verified-imperative → function+property), property-based-verification (5 exemplars: TLA+ + Dafny + Idris + PDDL + SVA), infrastructure-automation pentagram (Terraform + Docker + Nix + K8s + Ansible), reactive-UI (XState + SwiftUI + Elm), shell-family triad (Bash + jq + PowerShell), data-transformation family (6 exemplars), scientific-computing (6 exemplars: Fortran + NumPy + R + SQL-CTE + Julia + MATLAB), networked-API (4), **data-store (6 exemplars: SQL + GraphQL + jq + Protobuf + Redis + MongoDB)**, event-driven (4), error-handling (4), OO (3 variants), safety-critical-systems (4), STM/transactional-memory, dependent-types, script-language, array-golf (Forth+Perl+APL → density-inversion), **monadic-sugar (4 exemplars: Python-async + Scala-for-yield + Haskell-do + F#-computation-expr)**, **first-class-continuation (Scheme call/cc → 4 named patterns)**, **structured-imperative (5 exemplars: Pascal + Ada + COBOL + C + C++)**, **logic-programming (4 exemplars: Prolog + Datalog + Rego + SQL CTE)**. **Density-inversion + non-local-control-flow principles both documented**. **Unified (state-data, transition-fn, composition, property, screen) decomposition pattern proven across 7 traditionally-separate domains**. `screen` kind generalized to cover UI + diagrams + typeset documents. **Unified primitive set proven across ~70-year paradigm span** (Forth/COBOL/Fortran to Dafny/K8s/Ansible/Julia/Zig/Ada). **Key insight**: Nom's authoring-time contract vocabulary IS the Ada/SPARK/Dafny runtime-check vocabulary, just moved up the dependency stack from runtime to authoring-time. **Below the line is the 2026-04-13 original status carried forward; later rebases fold the above into a single "Where we are" section.**
