...
- **Nom takeaway:** Vietnamese aspect markers map to Nom's temporal flow semantics: `flow::once` (completed, idempotent), `flow::stream` (ongoing), `flow::scheduled` (prospective). Consider aspect-aware flow qualifiers.
  - ⏳ PLANNED — `flow::once`, `flow::stream`, `flow::scheduled` qualifiers not yet in the parser. Vietnamese keyword aliases (ASCII + diacritic) are shipped (commits `4b04b1d`, `5b59f82`) but aspect-marker semantics require runtime interpretation (Phase 5+).
