...
> **2026-04-14 late refresh:** HEAD `e013511`. **Session arc** since `fdfb32d`: 60+ commits across two macro-lanes. (1) **CoreNLP-inspired W4 strictness lane** — 5 of 6 wedges closed (A1/A2/A3/A4/A6; A5 audited with refactor deferred). A4 shipped as the full S1-S6 annotator pipeline (`nom_concept::stages` module with `run_pipeline(src) → PipelineOutput::{Nom, Nomtu}` + structured `StageFailure { stage, reason, detail, position }` diagnostics emitting NOMX-S<N>-<slug> ids). See doc 13 §5 + doc 18. (2) **Research finalization lane** — docs 13-19 landed: strictness plan, Accelworld translations (12 + gap list split), 100-repo harness plan, 35-row gap backlog triage, full authoring-guide chapter (doc 17 §I1-I13), annotator-pipeline design, deferred-design resolutions. **0 open design questions**. **nom-concept test total: 120** (was 77 at earlier snapshot). **Below the line is the 2026-04-13 original status carried forward; later rebases fold the above into a single "Where we are" section.**
