...
The pipeline `source → DB1/DB2 store → ConceptGraph closure → resolved refs → MECE objectives → Manifest JSON` runs end-to-end today. Two demos exercise it: `examples/concept_demo` (minimal) and `examples/agent_demo` (motivation 16 §5 killer-app seed: 6 tools + safety policy + intentional MECE collision). `examples/agent_demo_vn` has been deleted ([this-commit] — fully English vocabulary directive). e2e tests at `crates/nom-cli/tests/{concept_demo,agent_demo,build_manifest}_e2e.rs` (Windows gated because `nom` links LLVM-C.dll). Test totals: `nom-concept` reduced (~-15 VN tests removed), `nom-dict` 24, `nom-media` 50. Typed-slot `@Kind` refs resolve via `find_words_v2_by_kind` with alphabetical-smallest deterministic tiebreak; `with at-least N confidence` threshold syntax shipped; per-slot top-K diagnostic surfaces alternatives in `nom build status`.
