...
> **2026-04-14 very-late refresh:** HEAD `d051daa`. **Session arc** since `fdfb32d`: 110+ commits across three macro-lanes. (1) **CoreNLP-inspired W4 strictness lane** — 5 of 6 wedges closed (A1/A2/A3/A4/A6; A5 audited with refactor deferred). A4 shipped as the full S1-S6 annotator pipeline (`nom_concept::stages` module with `run_pipeline(src) → PipelineOutput::{Nom, Nomtu}` + structured `StageFailure { stage, reason, detail, position }` diagnostics emitting NOMX-S<N>-<slug> ids). See doc 13 §5 + doc 18. (2) **Research finalization lane** — docs 13-20 landed: strictness plan, Accelworld translations (**27** spanning 14+ paradigm families), 100-repo harness plan, **61-row** gap backlog triage, **complete authoring-guide chapter I1-I20**, annotator-pipeline design, deferred-design resolutions, session summary. **0 open design questions**. (3) **Pipeline parity push (a4c20-a4c38)** — every observable field from `parse_nom`/`parse_nomtu` now covered by `run_pipeline`; JSON round-trip (a4c35), earliest-stage-failure diagnostic contract (a4c36), strict-validator integration (a4c37), empty/whitespace-input safety (a4c38). Delegate migration fully unblocked. **nom-concept test total: 139** (was 77 at earlier snapshot; +62 this session). **Doc 16 backlog 27/61 closed (44%).** **Below the line is the 2026-04-13 original status carried forward; later rebases fold the above into a single "Where we are" section.**
