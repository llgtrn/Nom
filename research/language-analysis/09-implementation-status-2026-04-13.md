...
- **Evidence**: `4b04b1d` + `5b59f82` Vietnamese keyword aliases shipped in the lexer, then parked per user direction (keep but don't extend). deferred 09 §6 specifies the full standards stack (BCP 47, CLDR, UAX 31/15, UTS 39/55). **M3a** (commit `49869ab`) lands the `nom-locale` crate with `LocaleTag::parse` (BCP47), `normalize_nfc` (UAX #15), `LocalePack`+`RegisterMetadata`, baked `builtin_packs()` for vi-VN + en-US (both with empty `keyword_aliases`), `is_confusable` stub (returns `Deferred`), and `nom locale list` / `nom locale validate` CLI. **M3c** (commits `339a74a` + **reverted in `b3fe503`**) ships `apply_locale()` lexical pass with string-literal + comment skipping + bidirectional `to-canonical`/`from-canonical` + `nom locale apply --write/--json` CLI. Commit `339a74a` additionally baked 44 VN keyword aliases — **reverted by `b3fe503`** per scope correction. Tests: nom-locale 20/20 (infrastructure verified with a synthetic `xx-Test` fixture pack; vi-VN pack is a no-op pass-through). M3b remains: load UTS #39 confusables.txt to upgrade `is_confusable` from `Deferred` stub.
