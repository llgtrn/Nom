...
- **W4 strictness lane** (new 2026-04-14) — doc 13 specifies 6 wedges A1-A6. **A1 + A2 + A3 + A6 closed.** A3 lands as an additive `strict` module in nom-concept: `validate_nom_strict(&file)` + `validate_nomtu_strict(&file)` walk the AST post-parse and emit `StrictWarning { code, message, location }` for every typed-slot ref missing `with at-least N confidence`. Default parser unchanged; opt-in. 4 tests (s01-s04). **A6 audited as already locked** by pre-existing tests in `nom-cli/src/store/resolve.rs` — `typed_slot_two_candidates_picks_smallest_hash` and `typed_slot_three_candidates_propagates_matching_and_alternatives` already pin "N candidates ⇒ alphabetical-smallest wins + `stats.ambiguous += 1` + `alternatives` list carries the rest". No new code needed. Remaining: A4 (annotator-style staged parser, biggest), A5 (no-lossy-Option AST). See [doc 13 §5](13-nomx-strictness-plan.md#5-strictness-wedges-ordered).
