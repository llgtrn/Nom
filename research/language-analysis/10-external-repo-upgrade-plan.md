...
- **W4 strictness lane** (new 2026-04-14) — doc 13 specifies 6 wedges A1-A6. **A1 + A2 + A3 shipped.** A3 lands as an additive `strict` module in nom-concept: `validate_nom_strict(&file)` + `validate_nom_tu_strict(&file)` walk the AST post-parse and emit `StrictWarning { code, message, location }` for every typed-slot ref missing `with at-least N confidence`. Default parser unchanged; opt-in. 4 tests (s01-s04). Remaining: A4 (annotator-style staged parser, biggest), A5 (no-lossy-Option AST), A6 (reject-on-ambiguous in resolver). See [doc 13 §5](13-nomx-strictness-plan.md#5-strictness-wedges-ordered).
