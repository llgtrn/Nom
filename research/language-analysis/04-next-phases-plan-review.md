...
- 2026-04-13: second determinism canary (`compile_app_to_artifacts_is_deterministic` in nom-app) now proves byte-identical output across all 10 §5.12 OutputAspects over two builds of the same manifest. Both canaries gate the fixpoint track; they must stay green before any multi-stage self-build attempt.
- 2026-04-13: CI hardened — ubuntu runner installs `llvm-18 llvm-18-dev libpolly-18-dev`, sets `LLVM_SYS_180_PREFIX`, runs 10 nom-cli integration tests as hard gates (self-host parse gates, smoke, pipeline, meta, Rust↔Nom parity, parser subset probe). Determinism regressions fail CI at commit time, not at fixpoint attempt time.
...
4. ~~§10.3.2 parity track should be elevated in plan narrative from "housekeeping" to "real proof"; byte-fixpoint framed as aesthetic + pinned-toolchain-dependent.~~ **Applied 2026-04-13**: §10.3 intro now frames parity as primary correctness proof ("if forced to choose one, parity is the floor; fixpoint is the ceiling") and byte-fixpoint as aesthetic-plus-pinned-toolchain claim tied to the proof tuple's pin fields.
...
6. ~~Dependency-ordering fixes: Phase 9 LSP should precede or overlap §5.17 (since `nom check --audit` depends on it). §5.10 lifecycle should defer to after Phase 7 (canonicalizer is owned by Rust parser until Phase 7 ships Nom parser).~~ **Applied 2026-04-13**: dependency-ordering note added to the Phase 9 header in the plan; both reorderings documented in one paragraph with their rationales.
