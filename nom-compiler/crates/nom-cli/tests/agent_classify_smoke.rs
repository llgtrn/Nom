...
use nom_intent::react::{
    AgentAction, Observation, ReActAdapter, ReActBudget, ReActLlmFn, ReActStep, classify_with_react,
};
use nom_intent::{NomIntent, Reason};

fn tmp_dict() -> PathBuf {
    let p = std::env::temp_dir().join(format!("nom_agent_smoke_{}.db", std::process::id()));
...
fn classify_with_stub_llm_returns_reject_transcript() {
    ...
    let llm: ReActLlmFn =
        Box::new(|_prose, _transcript| Ok(ReActStep::Reject(Reason::Unparseable)));
...
fn nom_cli_adapter_drives_loop_to_completion_against_seeded_dict() {
    ...
    let llm: ReActLlmFn = Box::new(move |prose, transcript| adapter.next_step(prose, transcript));
...
fn nom_cli_adapter_rejects_on_empty_prose() {
    ...
    let adapter = NomCliAdapter::new();
    let llm: ReActLlmFn = Box::new(move |prose, transcript| adapter.next_step(prose, transcript));
    let transcript = classify_with_react("", &ReActBudget::default(), &llm, &tools).unwrap();
